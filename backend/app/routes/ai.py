...
from app.models import Monitor, Check, Analysis
...
def build_summary(checks) -> str:
    ...
    slow = sum(1 for c in checks if c.status == "SLOW")
...
def build_cross_summary(monitors, db) -> str:
    ...
    for m in monitors:
        ...
        if recent:
            avg = round(sum(c.response_time_ms for c in recent) / len(recent), 1)
...
@router.get("/{monitor_id}/analyses")
def get_analyses(
    monitor_id: int,
    db: Session = Depends(get_db),
    current_user: User = Depends(get_current_user)
